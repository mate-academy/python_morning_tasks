...
def test_different_simple_emails():
    ...
    assert num_unique_emails(emails) == 3, "Function should count each simple email as unique."
...
def test_emails_with_only_dots():
    ...
    assert num_unique_emails(emails) == 1, "Dots in the local part should not affect uniqueness."
...
def test_emails_with_only_plus():
    ...
    assert num_unique_emails(emails) == 1, "Plus signs in the local part should not affect uniqueness."
...
def test_emails_with_both_dot_and_plus_mixed():
    ...
    assert num_unique_emails(emails) == 1, "Mixing dots and plus signs should result in 1 unique email."
...
def test_emails_with_different_domains():
    ...
    assert num_unique_emails(emails) == 3, "Different domains should result in different emails."
...
def test_complex_emails():
    ...
    assert num_unique_emails(emails) == 1, "Complex emails with tags and sorting should count as one."


def test_emails_with_plus_at_end():
    emails = ["test.email+@leetcode.com", "test.email@leetcode.com"]
    assert num_unique_emails(emails) == 1, "Plus at the end of the local part should not affect uniqueness."


def test_emails_with_dot_at_start():
    emails = [".test.email@leetcode.com", "test.email@leetcode.com"]
    assert num_unique_emails(emails) == 1, "Dot at the start of the local part should not affect uniqueness."
