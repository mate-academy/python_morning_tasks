...
def test_single_element():
    nums = [5]
    assert (
        (result := sort_array_by_parity(nums)) == nums
    ), f"Failed: Result list should be the same as input one for nums = {nums}. Output: {result}"
...
def test_only_even_numbers():
    ...
    result = sort_array_by_parity(nums)
    assert (
        parity_checker(result, "even") is True
        and len(result) == evens
...
def test_only_odd_numbers():
    ...
    assert (
        parity_checker(result, "odd") is True
        and len(result) == ods
...
def test_zero_treats_as_even():
    ...
    assert 0 in result[:evens], f"Failed: Zero should be treated as even number for nums = {nums}"
